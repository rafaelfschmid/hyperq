	...
	*/
	...
	void execute1(){
		...
		for(auto f : functions){
			...
			(*kernels2)[k].computation = get<4>(f);

			//printf("kernels.computation=%d\n", (*kernels2)[k].computation);
			k++;
		}
...
int main(int argc, char **argv) {
	...
	for(int i = 0; i < number_of_kernels; i++) {
		...
		s.kernelCall(kernel3, num_threads, num_blocks, shared_size, (computation+1)*4);
	}
	...
	for(SharedVector::iterator iter = kernels2->begin(); iter != kernels2->end(); iter++)
	{
		...
		std::cout << iter->id << " " << iter->numOfThreads << " "<< iter->numOfBlocks << " "<< iter->sharedDynamicMemory << " "<< iter->computation << " "<< iter->milliseconds << "\n";
	}
